...
public:
    MatchingResult solve(const SatisfactionMatrix& matrix);
};

/**
 * @brief 算法结果对比结构体 - 存储两种算法的结果
 */
struct AlgorithmComparison {
    MatchingResult hungarian; // 匈牙利算法结果
    MatchingResult bruteForce; // 暴力搜索结果
    int n; // 问题规模
    double hungarianTime; // 匈牙利算法用时(ms)
    double bruteForceTime; // 暴力搜索用时(ms)
...
extern bool enable_verification;

/**
 * @brief 验证匈牙利算法的结果
 * @param matrix 满意度矩阵
 * @param hungarian_result 匈牙利算法的结果
 * @param hungarian_time 匈牙利算法用时(ms)
 * @return 对比结果，包含两种算法的详细比较
 */
AlgorithmComparison verify_hungarian_result(
    const SatisfactionMatrix& matrix,
    const MatchingResult& hungarian_result,
    double hungarian_time = 0.0);
